#include "..\Public\VIBuffer_Anim_Instance.h"

#include "GameInstance.h"

CVIBuffer_Anim_Instance::CVIBuffer_Anim_Instance(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CVIBuffer_Instancing{ pDevice, pContext }
{
}

CVIBuffer_Anim_Instance::CVIBuffer_Anim_Instance(const CVIBuffer_Anim_Instance& Prototype)
	: CVIBuffer_Instancing{ Prototype }
, m_pTextureCom{ Prototype.m_pTextureCom }
{
}

HRESULT CVIBuffer_Anim_Instance::Initialize_Prototype(const CVIBuffer_Instancing::INSTANCE_DESC& Desc, const _tchar* pModelFilePath, _fmatrix PreTransformMatrix)
{
...
HRESULT CVIBuffer_Anim_Instance::Initialize(void* pArg)
{
#pragma region INSTANCE_BUFFER
...
void CVIBuffer_Anim_Instance::Spread(_float fTimeDelta)
{
...
void CVIBuffer_Anim_Instance::SpreadGravity(_float fTimeDelta)
{
	D3D11_MAPPED_SUBRESOURCE	SubResource{};
...
void CVIBuffer_Anim_Instance::Fountain(_float fTimeDelta, _float fgravity)
{
	D3D11_MAPPED_SUBRESOURCE SubResource{};
...
void CVIBuffer_Anim_Instance::Drop(_float fTimeDelta)
{
	D3D11_MAPPED_SUBRESOURCE SubResource{};
...
HRESULT CVIBuffer_Anim_Instance::Bind_Texture(CShader* pShader, const _char* pConstantName)
{
	return m_pTextureCom->Bind_ShadeResource(pShader, pConstantName, 0);
}

HRESULT CVIBuffer_Anim_Instance::Read_File(const _tchar* pModelFilePath)
{
	FILE* file = nullptr;
...
HRESULT CVIBuffer_Anim_Instance::Safe_Release_Scene()
{
...
HRESULT CVIBuffer_Anim_Instance::Ready_VertexBuffer_NonAnim(const DATA_BINMESH* pAIMesh, _fmatrix PreTransformMatrix)
{
...
HRESULT CVIBuffer_Anim_Instance::Ready_Materials(const _tchar* pModelFilePath)
{
	if (nullptr == m_pBin_Scene)
		return E_FAIL;

	for (_uint i = 0; i < (_int)m_pBin_Scene->iMaterialCount; ++i)
	{
...
CVIBuffer_Anim_Instance* CVIBuffer_Anim_Instance::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext, const _tchar* pModelFilePath, _fmatrix PreTransformMatrix, const CVIBuffer_Instancing::INSTANCE_DESC& Desc)
{
	CVIBuffer_Anim_Instance* pInstance = new CVIBuffer_Anim_Instance(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype(Desc, pModelFilePath, PreTransformMatrix)))
	{
		MSG_BOX(TEXT("Failed to Created : CVIBuffer_Anim_Instance"));
...
CComponent* CVIBuffer_Anim_Instance::Clone(void* pArg)
{
	CVIBuffer_Anim_Instance* pInstance = new CVIBuffer_Anim_Instance(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX(TEXT("Failed to Cloned : CVIBuffer_Anim_Instance"));
		Safe_Release(pInstance);
	}
...
void CVIBuffer_Anim_Instance::Free()
{
	__super::Free();
